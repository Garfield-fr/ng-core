...
export class TranslateExtension {
  ...
  prePopulate(field: FormlyFieldConfig): void {
    ...
    if (to.addonRight?.text) {
      this._translate.onLangChange.subscribe(() => to.addonRight.text = this._translate.instant(to.addonRight.text));
    }
    if (to.addonLeft?.text) {
      this._translate.onLangChange.subscribe(() => to.addonLeft.text = this._translate.instant(to.addonLeft.text));
    }
  }
}
